	...
	public String detail() throws Exception{
		...
		if(org.apache.commons.lang.StringUtils.isNotEmpty(id)){
			...
			if(order!=null){
				...
				String chufang = "0";
				//是否含有处方药
				List<Integer> list = (List<Integer>) opensqlmanage.selectListByObject(Long.parseLong(id), "t_order.select_issuit_byorderid");
				for (Integer integer : list) {
					if(integer!=null &&(integer==2||integer==3)){
						chufang = "1";
					}
				}
				this.getRequest().setAttribute("chufang", chufang);
			}
		}
...
//	    }
	...
	public String toShippingAddressAdd(){
		return "shippingAddressAdd";
	}
	public String toShippingAddressSelect(){
	 ...
	 * @throws SQLException 
	 */
	public void ajaxSaveOrUpdateShippingAddress() throws Exception{
		TMember member = (TMember) this.getSession().getAttribute("member");
		Long memberId = member.getId();
		String publicServiceUrl = tsysparametermanager.getKeys("public_service_url");
		Map<String,String> publicMap = new HashMap<String,String>();
		...
		publicMap.put("mobile", memberReceiver.getMobile().trim());
		publicMap.put("receiver", memberReceiver.getReceiver().trim());
		publicMap.put("locationAddress",null);
		publicMap.put("isDefault",this.getRequest().getParameter("isDefault"));
		
		publicMap.put("address", memberReceiver.getAddress().trim());
		
		
		//根据citycode获取areaId,cityName
		String citycode = getRequest().getParameter("citycode");
		
		if(citycode != null && citycode.trim().length() > 0){
			CLocationCityExample example = new CLocationCityExample();
			example.createCriteria().andCitycodeEqualTo(citycode).andGradeEqualTo(2);
			List<CLocationCity> cityList =this.clocationcitymanager.selectByExample(example);
			if(cityList!=null&&cityList.size()>0){
				publicMap.put("areaId",String.valueOf(cityList.get(0).getId()));
			}
			publicMap.put("cityName", this.getRequest().getParameter("area"));
			publicMap.put("citycode", citycode);
		}else{//没有citycode 提示选择地区
		...
		String area =this.getRequest().getParameter("area").trim();
		
		positionMap.put("area", area);
		positionMap.put("locationAddress", null);
		positionMap.put("address", memberReceiver.getAddress().trim());
		publicMap.put("locationAddress",area.split("-")[area.split("-").length-1]);
		//String tempWsUrl = "http://test88.111yao.cn/111_yao_public/sltRouter?method=";
		String positionResultJsonStr = ClientSubmitPublic.getPublicService(positionMap, publicServiceUrl+"aMapGeoURIService");
		try{
		JSONObject positionResultJsonObj = JSONObject.fromObject(positionResultJsonStr);
		if(positionResultJsonObj != null){
			if(positionResultJsonObj.get("resCode") != null && (int)positionResultJsonObj.get("resCode") == 1){
				JSONObject resultJsonObj = JSONObject.fromObject(positionResultJsonObj.get("result"));
				String positionStr = (String) resultJsonObj.get("location");
				longitude = positionStr.split(",")[0];
				latitude = positionStr.split(",")[1];
		...
		System.out.println(positionResultJsonStr);
	 ...
	 * @throws SQLException 
	 */
	public void ajaxSaveOrUpdateReceiver1() throws Exception{
		TMember member = (TMember) this.getSession().getAttribute("member");
		Long memberId = member.getId();
		String publicServiceUrl = tsysparametermanager.getKeys("public_service_url");
		Map<String,String> publicMap = new HashMap<String,String>();
