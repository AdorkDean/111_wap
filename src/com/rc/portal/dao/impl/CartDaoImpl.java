...
import com.rc.portal.webapp.model.cart.CartPromModel;
...
//		Map<Long, List<CartPromModel>> CategoryMap = new HashMap<Long, List<CartPromModel>>();
		...
		for (CartPromModel m : cartlist) 
		{
			...
			total = total.add(m.getWap_price().multiply(new BigDecimal(m.getQuantity())));
//			map1.put(m.getGoods_id(), m.getQuantity());
		...
		for (CartPromModel cp : single)
		{
			...
			if(list.size()>0)
			{
				...
				if (item != null) {
					minmoney = minmoney.add((cp.getWap_price().subtract(item.getPrice())).multiply(new BigDecimal(cp.getQuantity())));
				}
			} 
			example.clear();
