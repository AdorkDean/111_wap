...
public class TGoodsPropertyDAOImpl implements TGoodsPropertyDAO {
    ...
    public int deleteByPrimaryKey(Long goodsid) throws SQLException {
        TGoodsProperty key = new TGoodsProperty();
        key.setGoodsid(goodsid);
        int rows = sqlMapClient.delete("t_goods_property.ibatorgenerated_deleteByPrimaryKey", key);
        return rows;
    }
    ...
    public TGoodsProperty selectByPrimaryKey(Long goodsid) throws SQLException {
        TGoodsProperty key = new TGoodsProperty();
        key.setGoodsid(goodsid);
        TGoodsProperty record = (TGoodsProperty) sqlMapClient.queryForObject("t_goods_property.ibatorgenerated_selectByPrimaryKey", key);
        return record;
    }
    ...
    public int updateByPrimaryKeySelective(TGoodsProperty record) throws SQLException {
        int rows = sqlMapClient.update("t_goods_property.ibatorgenerated_updateByPrimaryKeySelective", record);
        return rows;
    }

    public int updateByPrimaryKey(TGoodsProperty record) throws SQLException {
        int rows = sqlMapClient.update("t_goods_property.ibatorgenerated_updateByPrimaryKey", record);
        return rows;
    }
