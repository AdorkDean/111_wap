...
<section class="login-main">
    <article class="verification_info">
        <span>验证短信已发送至${tmember.mobile?default('')}</span>
    </article>
    ...
    <article class="login_btn">
        <a href="javascript:void(0)" id="registerButton">提交</a>
    </article>
</section>
...
$(function(){
	...
	$("#getSmsCode").click(function(){
		if(count == 61){
			$.ajax({
				url: "${base}/findpassword/findpassword!againGetMobileCode.action",
				type: "GET",
				data: {
					username: ${tmember.mobile?default('')},
				},
				cache: false,
	  ...
	  $("#registerButton").click(function(){
			...
			var username = ${tmember.mobile?default('')};
