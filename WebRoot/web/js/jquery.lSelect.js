		...
		lSelect: function(options) {
			...
			return this.each(function() {
				...
				var treePath = "";
					if(id!=null &&id!=""){
					$.ajax({
						url: settings.url.replace(".action","!getTreePathAndName.action"),
						type: settings.type,
						...
						success: function(data) {
							treePath = data.idPath;
						}
					});
				}
				var selectName = $input.attr("name") + "_select";
				var selectClass = $input.attr("name") + "_select_class";
				...
				function addSelect($position, parentId, currentId) {
					...
					var select = '<select class="'+selectClass+'" name="' + selectName + '">';
