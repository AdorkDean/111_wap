...
//				alert("请填写您的身份证号或输入正确的身份证号!");
			...
			if(thirdLocation==null||thirdLocation==""||thirdLocation=="请选择"){
				$alert("warn","请选择省市区");
//				alert("请选择省市区");
				return;
			}
...
$(function(){
	...
	function readFile(obj){
		...
		if(!/image\/jpeg/.test(file.type)){ 
			...
			$(obj).val("");
			$(obj).siblings("img").attr("src","/web/images/health-header-img.png");
			return false; 
		}
...
$(function(){
	var secoundFlag = 0;
	$("#goonWriteInfo").click(function(){
		if(secoundFlag==0){
			var headFile = $("#headFile").val();
			var realName = $("input[name='realName']").val();//真实姓名
			var nickName = $("input[name='nickName']").val();//昵称
...
//		var cardCode = $("input[name='cardCode']").val();//身份证号
			...
			if(thirdLocation==null || thirdLocation ==""){
				$alert("warn","请先选择您的省市区");
				return ;
			}
			if(address==null || $.trim(address) ==""){
				$alert("warn","请输入您的详细地址");
				return ;
			}
			secoundFlag = 1;
			autoTimeBtn_goon(this);
			$("#goonWriteInfo").attr("disabled","disabled");
			$("#goonWriteInfo").attr("style","background-color:#777777;");
			$("#subForm").submit();
		}
	});
